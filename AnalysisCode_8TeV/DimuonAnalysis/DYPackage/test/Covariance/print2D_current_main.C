...
0.000957856, 0.00111243, 0.00119348, 0.00095393, 0.000971326, 0.000964124, 0.000861841, 0.000586147, 0.000449087, 0.000277411, 0.000177376, 3.9162e-05};
 ...
 cout << "Check diagonals consistent with paper" << endl;
 for (int i = 0; i < 132; i++) {
   for (int j = 0; j < 132; j++) {
     if (i!=j) continue;
     double val = 0;
     ...
     if (norm_type == "absolute") {
          if (fsrFlag == "pre") val = sqrt(sigma_Z*sigma_Z*diagonals_preFSR[i]*diagonals_preFSR[j] + rdiagonals_preFSR[i]*rdiagonals_preFSR[j]*sigmaZerrNoLumi);
          if (fsrFlag == "post")  val = sqrt(sigma_Z*sigma_Z*diagonals_postFSR[i]*diagonals_postFSR[j] + rdiagonals_postFSR[i]*rdiagonals_postFSR[j]*sigmaZerrNoLumi);
     } 
     if (norm_type == "normalized" || norm_type == "absolute") {
