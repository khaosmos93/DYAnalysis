...
void plot_MuMu(void) {
  ...
  gStyle->SetPadGridY(kFALSE);
   ...
   //SET UP FINE THEORY
   ifstream in_xsec_massNNLO4("../style/root/Plain/NNLO/8TeV_CTEQ12NNLO_41.txt",ios::in);
   ...
   for( int i = 0; i < nbins4-1; i++ ) {
     //cout << i+1 << " " << mass_xbin4[i] << " - " << mass_xbin4[i+1] << endl;
     double dm_ = mass_xbin4[i+1]-mass_xbin4[i];

     //NNLO  
     in_xsec_massNNLO4 >> xsec_mass4[i] >>  xsec_err_mass4[i] ;
     //FIXME Alexey correction to remove the kink at high mass in the theory plot
   ...
   for (int i = 0; i < 41; i++) {
      ...
      //FIXME
      //theory->SetBinError(i+1,theory->GetBinError(i+1)/theory->GetBinWidth(i+1));
   }
   ...
   if (!IS_RSHAPE) {
      ...
      else g = new TFile("../Outputs/absex_full_PI_Bayesian.root");
   } else { 
   ...
   for (int i = 0; i < 41; i++) {
      cout << data->GetBinContent(i+1) << " " << data->GetBinError(i+1) << endl;
   }
...
//FIXME need ratio plots
  ...
  h_theory4->GetYaxis()->SetRangeUser(h_theory4->GetMinimum()/10.,h_theory4->GetMaximum()*100.);
  h_theory4->Draw("L");
  ...
  p2->SetTickx(kFALSE);
  TAxis* ax_ = ratio->GetXaxis();
  TAxis* ay_ = ratio->GetYaxis();

  ax_->SetNdivisions(0);
  ax_->SetTitleOffset(0.96);
  ax_->SetLabelSize(0);
  ...
  ratio->SetMaximum(1.7); //1.52);
  ratio->SetMinimum(0.3);//0.4);
...
{         
  ...
  float mass[7] = {15,30,60,120,240,600,2000};
