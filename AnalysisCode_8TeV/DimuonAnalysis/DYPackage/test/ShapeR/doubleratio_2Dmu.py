...
fsyst_effMC = TFile('../Inputs/acc/out1_JulyRep_w1_TRMNov_41.root')
...
f_mu1D = TFile('../Outputs/absex_DET2D_Bayesian.root') #absex_DET2D_comb_PI.root')
...
fsyst_effMC7 = TFile('/group/cms/users/asvyatko/CMSSW_4_2_8/src/DYPackage/Inputs/acc/out.root')
...
for i in range(hratio.GetXaxis().GetNbins()):
      ...
      #hratio.SetBinContent(i+1,hratio.GetBinContent(i+1)+gRandom.Gaus(0,0.1*hratio.GetBinError(i+1)))

fout = TFile('doubleratio_2Dmu_Bayesian.root','recreate')
hratio.Write('hxsec')
fout.Close()
