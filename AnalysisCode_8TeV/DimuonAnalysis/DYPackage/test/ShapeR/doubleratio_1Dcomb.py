...
f_mu1D = TFile('../Outputs/absex_full_comb_Bayesian.root')  #absex_full_comb_PI.root')
...
#get rshape at 7 TeV
fmu_7TeV = TFile("../Outputs/rshape_full2011_AndriusHepData.root")
...
#patch
hratio.SetBinContent(41,hratio.GetBinContent(41)/1.07)
...
for i in range(hstat78.GetNbinsX()):
    hstat78.SetBinContent(i+1,comb_fract*sqrt(pow(hstat7.GetBinContent(i+1),2)+pow(hstat8.GetBinContent(i+1),2)))
...
for i in range(hstat78.GetNbinsX()):
    ...
    #all the systematics with ACC double counting
    #total_unc.SetBinContent(i+1,sqrt(pow(acc_syst78.GetBinContent(i+1),2)+pow(pdf_syst78.GetBinContent(i+1),2)+pow(mod_syst78.GetBinContent(i+1),2)+
    #pow(FSR_syst78.GetBinContent(i+1),2)+pow(bkg_syst78.GetBinContent(i+1),2)+pow(unf_syst78.GetBinContent(i+1),2)+pow(effcorr_syst78.GetBinContent(i+1),2)+pow(hstat78.GetBinContent(i+1),2)))
    #no acceptance and modeling, PDF
    #total_unc.SetBinContent(i+1,sqrt(pow(FSR_syst78.GetBinContent(i+1),2)+pow(bkg_syst78.GetBinContent(i+1),2)+pow(unf_syst78.GetBinContent(i+1),2)+pow(effcorr_syst78.GetBinContent(i+1),2)+pow(hstat78.GetBinContent(i+1),2)))
...
for i in range(hratio.GetXaxis().GetNbins()):
      #if i > 26: hratio.SetBinError(i+1, hratio.GetBinContent(i+1)*sqrt(pow(total_unc.GetBinContent(i+1),2)+pow(0.01,2)))
      #else: hratio.SetBinError(i+1, hratio.GetBinContent(i+1)*sqrt(pow(total_unc.GetBinContent(i+1),2)+pow(0.04,2)))
      hratio.SetBinError(i+1, hratio.GetBinContent(i+1)*sqrt(pow(total_unc.GetBinContent(i+1),2)))


fout = TFile('doubleratio_1Dcomb_full_Bayesian.root','recreate')
hratio.Write()
fout.Close()
