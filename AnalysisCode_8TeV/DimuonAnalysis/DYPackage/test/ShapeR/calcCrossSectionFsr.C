...
//FIXME control unfolding
const TFsrCorrectionType_t fsrCorrection_BinByBin= _fsrCorr_unfGood; //_fsrCorr_binByBin; //_fsrCorr_unfGood;
...
void  efficiencyCorrection(const TMatrixD &vin, const TMatrixD &vinStatErr, const TMatrixD &vinSystErr,
             TMatrixD &vout, TMatrixD &voutStatErr, TMatrixD &voutSystErr);
void  efficiencyCorrectionDD(const TMatrixD &vin, const TMatrixD &vinStatErr, const TMatrixD &vinSystErr,
             TMatrixD &vout, TMatrixD &voutStatErr, TMatrixD &voutSystErr); 
void  efficiencyCorrectionTruth(const TMatrixD &vin, const TMatrixD &vinStatErr, const TMatrixD &vinSystErr,
             TMatrixD &vout, TMatrixD &voutStatErr, TMatrixD &voutSystErr); 
...
void calcCrossSectionFsr(const TString conf) { //="../config_files/xsecCalc.conf")
  ...
  TMatrixD effCorrectedYieldsDD(DYTools::nMassBins,nMaxYBins);
  TMatrixD effCorrectedYieldsStatErrDD(DYTools::nMassBins,nMaxYBins);
  TMatrixD effCorrectedYieldsSystErrDD(DYTools::nMassBins,nMaxYBins);
...
//FIXME this is the unfolding test routine 1
  // Apply efficiency correction
/*
  std::cout << "efficiency correction\n";
  efficiencyCorrection(signalYields,signalYieldsStatErr,signalYieldsSystErr, //unfoldedYields, unfoldedYieldsStatErr, unfoldedYieldsSystErr,
  ...
  applyUnfolding(accCorrectedYields,accCorrectedYieldsStatErr,accCorrectedYieldsSystErr, //signalYields, signalYieldsStatErr, signalYieldsSystErr,
...
//FIXME this is the unfolding test routine 2
  // Apply efficiency correction
  std::cout << "efficiency correction\n";
  efficiencyCorrectionDD(signalYields,signalYieldsStatErr,signalYieldsSystErr, //unfoldedYields, unfoldedYieldsStatErr, unfoldedYieldsSystErr,
                       effCorrectedYieldsDD, effCorrectedYieldsStatErrDD, effCorrectedYieldsSystErrDD);
  saveYields(effCorrectedYieldsDD,effCorrectedYieldsStatErrDD,effCorrectedYieldsSystErrDD, triggers, "debug_effCorr",0);

  // Apply mass resolution unfolding
  std::cout << "apply unfolding\n";
  applyUnfolding(effCorrectedYieldsDD,effCorrectedYieldsStatErrDD,effCorrectedYieldsSystErrDD,
                 unfoldedYields, unfoldedYieldsStatErr, unfoldedYieldsSystErr);
  saveYields(unfoldedYields,unfoldedYieldsStatErr,unfoldedYieldsSystErr, triggers,"debug_unf",0);
  
  // Apply efficiency correction
  std::cout << "efficiency correction\n";
  efficiencyCorrectionTruth(unfoldedYields, unfoldedYieldsStatErr, unfoldedYieldsSystErr,
                       effCorrectedYields, effCorrectedYieldsStatErr, effCorrectedYieldsSystErr);
  saveYields(effCorrectedYields,effCorrectedYieldsStatErr,effCorrectedYieldsSystErr, triggers, "debug_effCorr",0);
  ...
  acceptanceCorrection(effCorrectedYields, effCorrectedYieldsStatErr, effCorrectedYieldsSystErr,
                       accCorrectedYields, accCorrectedYieldsStatErr, accCorrectedYieldsSystErr);
  saveYields(accCorrectedYields,accCorrectedYieldsStatErr,accCorrectedYieldsSystErr, triggers, "debug_effAccCorr",0);
//FIXME below is the main routine
*/
  // Apply unfolding
  std::cout << "apply unfolding\n";
...
void readData(TMatrixD &v, TMatrixD &vErr1, TMatrixD &vErr2){
  ...
  for(int i=0; i<DYTools::nMassBins; i++){
    for (int yi=0; yi<DYTools::nYBins[i]; yi++) {
      ...
      //std::cout << "In read data Index " << i << " " << yi << " " << YieldsSignal[i][yi] << std::endl;
             ...
             TMatrixD &vout, TMatrixD &voutStatErr, TMatrixD &voutSystErr)
{
  ...
  //FIXME smooth the efficiency correction
  TH1F* hrhoDataMcPtr = new TH1F(*rhoDataMcPtr);
  TVectorD rhoDataMc= *rhoDataMcPtr;
  for (int ii = 0; ii < hrhoDataMcPtr->GetNbinsX(); ii++) {
      rhoDataMc[ii] = hrhoDataMcPtr->GetBinContent(ii+1);
  }
  ...
  for(int mi=0, idx=0; mi<DYTools::nMassBins; mi++){
    for (int yi=0; yi<DYTools::nYBins[mi]; ++yi,++idx) {
      double effFactor =rhoDataMc[idx];
      double effErr = effFactor
	* sqrt(rhoDataMcErr[idx]*rhoDataMcErr[idx]/rhoDataMc[idx]/rhoDataMc[idx]);
             ...
             TMatrixD &vout, TMatrixD &voutStatErr, TMatrixD &voutSystErr)
{
  ...
  for(int mi=0, idx=0; mi<DYTools::nMassBins; mi++){
    for (int yi=0; yi<DYTools::nYBins[mi]; ++yi,++idx) {
      double effFactor = efficiencyArray[mi][yi];
      double effErr = effFactor
		    ...
		    const TriggerSelection &triggers, const TString specTag)
{
  ...
  for(int i=0; i<DYTools::nMassBins; i++) {
    for (int yi=0; yi<DYTools::nYBins[i]; ++yi) {
      vout[i][yi] = vin[i][yi] / mylumi;
      voutStatErr[i][yi] = vinStatErr[i][yi] / mylumi;
      voutSystErr[i][yi] = vinSystErr[i][yi] / mylumi;
  ...
  //vin.Write("Init");
  //vinStatErr.Write("InitErr");
  //vinSystErr.Write("InitSystErr");
  vout.Write("XSec");
  voutStatErr.Write("XSecErr");
  voutSystErr.Write("XSecSystErr");
		       ...
		       const TriggerSelection &triggers, const TString specTag)
{
  ...
  for (int iM=0; iM<DYTools::nMassBins; ++iM) {
    for (int iY=0; iY<DYTools::nYBins[iM]; ++iY) {
      ...
      double binw = 0.1; 
      if (iM == DYTools::nMassBins-1) binw = 0.2;

      normXSecByBin[iM][iY]=voutNorm[iM][iY]/binw;
      normXSecErrByBinStat[iM][iY]=voutNormStatErr[iM][iY]/binw;
      normXSecErrByBinSyst[iM][iY]=voutNormSystErr[iM][iY]/binw;
      normXSecErrByBin[iM][iY]=sqrt( SQR(voutNormStatErr[iM][iY]) + SQR(voutNormSystErr[iM][iY]) )/binw;
...
void printRelativeSystErrorsForCovarianceMatrix(){
  ...
  cout << "XXX fnameFsrCorrectionConstantsBbB " << fnameFsrCorrectionConstantsBbB << endl;
  assert(fileConstants.IsOpen());
  TMatrixD *fsrCorrectionMatrixPtr  = (TMatrixD *)fileConstants.FindObjectAny("fsrCorrectionMatrix");
  TMatrixD *fsrCorrectionErrMatrixPtr = (TMatrixD *)fileConstants.FindObjectAny("fsrCorrectionErrMatrix");
  assert(fsrCorrectionMatrixPtr && fsrCorrectionErrMatrixPtr);
