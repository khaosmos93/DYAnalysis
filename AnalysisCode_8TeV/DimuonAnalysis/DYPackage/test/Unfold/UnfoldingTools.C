...
#include "TUnfold.h"
#include "TRandom.h"
#include "/afs/cern.ch/work/a/asvyatko/NEWDY/CMSSW_5_3_11/src/DimuonAnalysis/DYPackage/test/Unfold/RooUnfold-1.1.1/src/RooUnfoldSvd.h"
#include "/afs/cern.ch/work/a/asvyatko/NEWDY/CMSSW_5_3_11/src/DimuonAnalysis/DYPackage/test/Unfold/RooUnfold-1.1.1/src/RooUnfoldBayes.h"
#include "/afs/cern.ch/work/a/asvyatko/NEWDY/CMSSW_5_3_11/src/DimuonAnalysis/DYPackage/test/Unfold/RooUnfold-1.1.1/src/RooUnfoldResponse.h"
#endif
 ...
 int  unfoldTrueToReco(const TVectorD &vin, TVectorD &vout, const TString &unfoldingConstFileName)
  {
    ...
    // Read unfolding constants
    std::cout << "1unfoldTrueToReco(V): Load constants from <" << unfoldingConstFileName 
	      << ">" << std::endl;
...
//-----------------------------------------------------------------
  ...
  int  unfold(const TMatrixD &vinM, TMatrixD &voutM, const TString &unfoldingConstFileName, TVectorD &vin, TVectorD &vout)
  {
    ...
    bool doCustomInvertInPlace = false; //true <-- default;
    ...
    cout << "unfoldingConstFileName " << unfoldingConstFileName << endl;
    TString trueFileName = "yieldsMcPostFsrGenFIArray";
    TString preunfoldedMCFileName = "yieldsMcPostFsrRecFIArray";

    cout << "Name " << unfoldingConstFileName << endl;
    if (unfoldingConstFileName.Contains("fsr") && !unfoldingConstFileName.Contains("DET")) {
        trueFileName = "yieldsMcPreFsrGenFIArray";
        preunfoldedMCFileName = "yieldsMcPostFsrGenFIArray";
    } else if (unfoldingConstFileName.Contains("fsr") && unfoldingConstFileName.Contains("DET")) {   
        trueFileName = "yieldsMcPreFsrGenDETFIArray";
        preunfoldedMCFileName = "yieldsMcPostFsrGenDETFIArray";
    }

    TMatrixD DetResponse             = *(TMatrixD *)fileConstants.FindObjectAny("DetResponse");
    TMatrixD DetResponseErrPos       = *(TMatrixD *)fileConstants.FindObjectAny("DetResponseErrPos");
    TMatrixD DetResponseErrNeg       = *(TMatrixD *)fileConstants.FindObjectAny("DetResponseErrNeg");
    TMatrixD DetInvertedResponse     = *(TMatrixD *)fileConstants.FindObjectAny("DetInvertedResponse");
    TMatrixD DetInvertedResponseErr  = *(TMatrixD *)fileConstants.FindObjectAny("DetInvertedResponseErr");
    TMatrixD DetMigration            = *(TMatrixD *)fileConstants.FindObjectAny("DetMigration");
    TMatrixD DetMigrationErr         = *(TMatrixD *)fileConstants.FindObjectAny("DetMigrationErr");
    TVectorD preunfoldedMC           = *(TVectorD *)fileConstants.FindObjectAny(preunfoldedMCFileName);
    TVectorD trueMC                  = *(TVectorD *)fileConstants.FindObjectAny(trueFileName);

    //Histograms as input
    //true yields: postFSR gen for mass resoution, preFSR gen for FSR
    TH1D* _inTrue = new TH1D(trueMC);
    //measured MC - preunfolded MC: post-FSR reco for mass resolution, postFSR gen for FSR
    TH1D* _inMeasMC = new TH1D(preunfoldedMC);

    // Pack the matrix (was there by default)
    if (!flattenMatrix(vinM, vin)) return 0;

    //Default unfolding
    vout = 0;
    int nBins = DYTools::getTotalNumberOfBins();
    TMatrixD DetInvertedResponseCustom = *(TMatrixD *)fileConstants.FindObjectAny("DetResponse");
    TH2D* tmp = new TH2D(DetInvertedResponseCustom);
    if (doCustomInvertInPlace) {
      if (unfoldingConstFileName.Contains("fsr")) {
        //suppress off diagonals
        for(int i=0; i<nBins; i++){
          for(int j=0; j<nBins; j++){
            //test1 
            //if (fabs(i-j) > 10) DetInvertedResponseCustom(j,i) = 0; 
          }
        }
      }
      DetInvertedResponseCustom.Invert();
    } 

    for(int i=0; i<nBins; i++){
      for(int j=0; j<nBins; j++){
	if (!doCustomInvertInPlace) vout[i] += DetInvertedResponse(j,i) * vin[j];
        else { 
           vout[i] += DetInvertedResponseCustom(j,i) * vin[j];
           //FIXME closur test
           //vout[i] += DetInvertedResponseCustom(j,i) *_inMeasMC->GetBinContent(j+1);
        }
      }
    }

    //Matrix as input
    //Event matrix (transpose, because of the way matrix-TH2 works)
    TH2D* _inEvMat_Transpose = new TH2D(DetMigration); //DetResponse); //isn't it just the migration matrix 
    //observed pre-unfolded yield
    TH1D* _inMeasData = new TH1D(vin);

    //simple unfolded yield for further comparison 
    TH1D* unfolded_simple = new TH1D(vout);

    //Transpose back the event matrix
    TH2D* _inEvMat = (TH2D*)_inEvMat_Transpose->Clone();
    for (int ix = 0; ix < _inEvMat->GetNbinsY(); ix++) {
         for (int iy = 0; iy < _inEvMat->GetNbinsY(); iy++) {
            _inEvMat->SetBinContent(ix+1,iy+1,_inEvMat_Transpose->GetBinContent(iy+1,ix+1));
         }
    }

    RooUnfoldResponse response(_inMeasMC,_inTrue,_inEvMat);
    RooUnfoldBayes unfoldBayes(&response, _inMeasData, 1);
    RooUnfoldBayes unfoldBayesMC(&response, _inMeasMC, 1);
    TH1D* unfolded = (TH1D*) unfoldBayes.Hreco();
    TH1D* unfoldedMC = (TH1D*) unfoldBayesMC.Hreco();
 
    //save for further reference
  if (!unfoldingConstFileName.Contains("fsr") && !unfoldingConstFileName.Contains("DET")) {
    TFile* fout1 = new TFile("debug.root","recreate");
    fout1->cd();
    _inEvMat->Write("ev_matrix");
    _inTrue->Write("trueMC");
    _inMeasMC->Write("preunfoldedMC");
    unfolded->Write("unfolded");
    unfoldedMC->Write("unfoldedMC");
    unfolded_simple->Write("unfolded_simple");
    _inMeasData->Write("preunfolded");
    fout1->Close();
  }

  if (unfoldingConstFileName.Contains("fsr") && unfoldingConstFileName.Contains("DET")) {

    TFile* fout1 = new TFile("debug2.root","recreate");
    fout1->cd();
    _inEvMat->Write("ev_matrix");
    _inTrue->Write("trueMC");
    _inMeasMC->Write("preunfoldedMC");
    ...
    //for(int i=0; i<nBins; i++) {
    //   //if (useAlternativeForResult && !unfoldingConstFileName.Contains("fsr")) vout[i] = unfolded->GetBinContent(i+1);
    //   if (useAlternativeForResult) vout[i] = unfolded->GetBinContent(i+1);
    //}
