...
//mass value until which do reweighting 
{
  ...
  for(UInt_t ifile = 0; ifile<fnamev.size(); ifile++) {
    ...
    hZMassv.push_back(new TH1F(hname,"",2000,0,2000)); 
    hZMassv[ifile]->Sumw2();
    hZEtav.push_back(new TH1F(hname,"eta",360,-3.,3.));
    hZEtav[ifile]->Sumw2();
  }
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    for(UInt_t ientry=0; ientry<eventTree->GetEntries(); ientry++) {
      ...
      if (info->lumiSec == 124146 || info->lumiSec == 124147 || info->lumiSec == 124151) continue;
	...
	if (new_fewz_code) { 
          ...
          //if (gen->vmass < 19.) cout << "fewz_weight " << gen->vy << " " << gen->vpt << " " << fewz_weight << endl;
	} else
        {
...
//       printf("mass= %f   pt= %f    Y= %f     weight= %f\n",gen->mass, gen->vpt, gen->vy, fewz_weight);
	...
	if(ibinMass != -1 && ibinMass < nPassv.GetNrows()){
      ...
      //if (mass < 20 || mass > 30) continue; 
      //if (!(fabs(gen->eta_1) > 1.4442 && fabs(gen->eta_1) < 1.566)) 
      hZEtav[ifile]->Fill(gen->eta_1);
      //if (!(fabs(gen->eta_2) > 1.4442 && fabs(gen->eta_2) < 1.566)) 
      hZEtav[ifile]->Fill(gen->eta_2);
  ...
  //FIXME shortcut to output
  TFile* facc = new TFile("test_acc.root","recreate");
  facc->cd();
  for(UInt_t i=0; i<fnamev.size(); i++) {
     hZEtav[i]->Write();
     hZMassv[i]->Write();
