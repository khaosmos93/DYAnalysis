...
int eMu::run(){
  ...
  for(sIter iter_path = s_pathv.begin(); iter_path !=s_pathv.end(); ++iter_path){
     ...
     std::cout << "Path " << *iter_path << std::endl;
	...
	if (doPUreWeight) {
	  pu_weight= puWeight->getWeightHildreth( nGoodPV );
	}

        //fill histo with events
	if (isData) {
          //std::cout << "Mass, data " << mass << " " << weight << " " << *iter << std::endl;
  ...
  if (saveToRootFile){
    ...
    if (!doDMDY){
      ...
      for (int i = 0; i < num_massBins2D; ++i){
        ...
        eMuDMDYTot[massB]->Write();
        data_dmdyHistv[massB].at(0)->SetName("data_dmdyHistv"+TString(s.str().c_str()));
        data_dmdyHistv[massB].at(0)->Write();
      }
    }
  }
