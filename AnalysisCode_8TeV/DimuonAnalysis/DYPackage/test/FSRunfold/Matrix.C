...
//  gROOT->ForceStyle(true);
  ...
  //TFile* f = new TFile("FSRresMatrixProd.root");
  //TFile* f = new TFile("rootfiles/resMatrixProd_"+_trig+"_40bins_fullpowheg_corr.root","open");
  TFile* f  = new TFile("../Inputs/FSR/FSRresMatrixProd_tmp.root");
  f->cd();
  TH2F* hmig = (TH2F*)hden2->Clone();
  TH1F* _inTrue = (TH1F*)hpreFSR2->Clone();
  TH1F* _inMeas = (TH1F*)hpostFSR2->Clone();
...
/*
  ...
  hmig->Draw("COLZ");
  c1->Print("eps/migMatrix_"+_trig+".png");
*/
  ...
  //_inResMat->SetTitle("CMS at 7TeV, Lumi = 36.1 pb^{-1}");
  _inResMat->GetXaxis()->SetTitle("pre-FSR mass/rapidity bins");  //generated M(#mu#mu) post-FSR [GeV]");
  _inResMat->GetYaxis()->SetTitle("post-FSR mass/rapidity bins"); //reconstructed M(#mu#mu) [GeV]");
  _inResMat->GetXaxis()->SetMoreLogLabels();
  //_inResMat->GetYaxis()->SetMoreLogLabels();
  _inResMat->GetYaxis()->SetNoExponent();
  _inResMat->GetXaxis()->SetNoExponent();
  _inResMat->SetMinimum(0.00025);
  _inResMat->Draw("COLZ");
  c2->Print("eps/resMatrix_"+_trig+".png");
/*
  ...
  c3->Print("eps/inResMatrix_"+_trig+".png");
*/
}
