#define EleEventSelector_FSRUnf_cxx
// The class definition in EleEventSelector_FSRUnf.h has been generated automatically
...
//
// Root > T->Process("EleEventSelector_FSRUnf.C")
// Root > T->Process("EleEventSelector_FSRUnf.C","some options")
// Root > T->Process("EleEventSelector_FSRUnf.C+")
//

#include "EleEventSelector_FSRUnf.h"
...
EleEventSelector_FSRUnf::EleEventSelector_FSRUnf(TTree* )
{
...
EleEventSelector_FSRUnf::~EleEventSelector_FSRUnf() { }


void EleEventSelector_FSRUnf::Begin(TTree*)
{
...
void EleEventSelector_FSRUnf::SlaveBegin(TTree*)
{
...
Bool_t EleEventSelector_FSRUnf::Process(Long64_t entry)
{
   ...
   // It can be passed to either EleEventSelector_FSRUnf::GetEntry() or TBranch::GetEntry()
    ...
    //reset weights
     ...
     if (filename.Contains("Data")) {
      ...
      if (isMC && !filename.Contains("QCD")) {
        ...
        int GENIndex = -1;
         for( int j = 0; j < GENnPair; j++ ) {
          //if( (filename.Contains("DYM20") && !filename.Contains("DYM200")) && GENElectron1_mother[j] != 23 ) continue;
          //if( (filename.Contains("DYM20") && !filename.Contains("DYM200")) && GENElectron2_mother[j] != 23 ) continue;
          if( GENElectron1_status[j] != 3 ) continue;
          if( GENElectron2_status[j] != 3 ) continue;
      ...
      if ((filename.Contains("DYM200") && !filename.Contains("DYM2000")) && genMass > 400) { continue; }
       ...
       genDiMuPt = sqrt((GENElectron1_Px[GENIndex]+GENElectron2_Px[GENIndex])*(GENElectron1_Px[GENIndex]+GENElectron2_Px[GENIndex])+(GENElectron1_Py[GENIndex]+GENElectron2_Py[GENIndex])*(GENElectron1_Py[GENIndex]+GENElectron2_Py[GENIndex]));
       //look up FEWZ weight
       ...
       for( int j = 0; j < GENnPair; j++ ) {
           ...
           if( fabs(GENElectron1_eta[j]) < 2.4 && fabs(GENElectron2_eta[j]) < 2.4 ) {
              if( (GENElectron1_pT[j] > 20 && GENElectron2_pT[j] > 10) || (GENElectron2_pT[j] > 20 && GENElectron1_pT[j] > 10) ) {
               inAcc= true; 
              }
            }  
            if( inAcc ) {
             if( GENElectron1_status[j] == 1 && GENElectron2_status[j] == 1 ) {
              inAccPostFSR = true;
              simIndex = j;
            }
       ...
       if (!(inAccPostFSR && inAccPreFSR)) continue;
        ...
        if( inAccPostFSR && inAccPreFSR) { 
          ...
          hpreFSR_corr->Fill(GENMass, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
          hpostFSR_corr->Fill(simMass, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
          hden_corr->Fill(GENMass, simMass, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
          ...
          hpreFSR_corr2->Fill(val_gen, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
          hpostFSR_corr2->Fill(val_sim, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
          hden_corr2->Fill(val_gen, val_sim, WEIGHT*FEWZ_WEIGHT*PU_WEIGHT);
        }
...
void EleEventSelector_FSRUnf::SlaveTerminate()
{
...
void EleEventSelector_FSRUnf::Terminate()
{
...
//dynamic_cast<TPair*>(fInput->FindObject("PROOF_CurrentElement")))) {
   ...
   TFile fout(TString(dName)+"EE.root","recreate");
   Nntuple->Write();
   hpreFSR->Write();
   hpostFSR->Write();
   hden->Write();
