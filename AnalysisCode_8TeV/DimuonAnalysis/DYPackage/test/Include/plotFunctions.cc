...
//for "actual binning": hMassv -> hMassBinsv, hMassDibosons -> hMassBinsDibosons
{
  ...
  if (actualBinning)
    {
      hMassDibosons->GetXaxis()->SetRangeUser((DYTools::study2D) ? 20. : 15.,2000);
  ...
  for(UInt_t isam=(hasData) ? 1:0; isam<samplev.size(); isam++){
    ...
    if (actualBinning)
    {
      hMassv[isam]->GetXaxis()->SetRangeUser((DYTools::study2D) ? 20. : 15.,2000);
		   ...
		   TFile *histoFile) 
{
  ...
  TLine *lineAtOne = new TLine(15.0, 1.0, 2000, 1.0);
