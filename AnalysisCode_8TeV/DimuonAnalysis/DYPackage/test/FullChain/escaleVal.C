void escaleVal() { //TString Run) {
  ...
  TFile* fdata_es = new TFile("yields1D.root");
  fdata_es->cd();
  ...
  hmc_es->SetLineColor(kBlue);
  hmc_es->SetFillColor(kBlue);
  hmc_es_bb->SetLineColor(kBlue);
  hmc_es_ee->SetLineColor(kBlue);
  hmc_es_oo->SetLineColor(kBlue);
  hmc_es_bb->SetFillColor(kBlue);
  hmc_es_ee->SetFillColor(kBlue);
  hmc_es_oo->SetFillColor(kBlue);

  //combine all the classes
  hmc_es_bb->Add(hmc_es_ee);
  hmc_es_bb->Add(hmc_es_oo);

  hdata_es_bb->Add(hdata_es_ee);
  hdata_es_bb->Add(hdata_es_oo);

  std::cout << hdata_es->Integral() << hmc_es->Integral() << std::endl;
...
//FIXME need ratio plots
  ...
  hmc->SetMinimum(5);
  //hmc->GetYaxis()->SetNdivisions(503);
  hmc->Draw("E2");
  hdata->Draw("EPsame");
  ...
  for (int i = 0; i < ratio_bb->GetNbinsX(); i++) {
     cout << i+1 << " " << ratio_bb->GetBinContent(i+1) << endl;
  }


  TLegend* leg = MakeLegend2();
  leg->AddEntry(hdata,"Data, corrected","lp");
  leg->AddEntry(hmc,"MC, corrected","f");
  leg->Draw("same");
  TPaveText* pt = MakeTPave();
  if (etaclass == "bb") pt->AddText("Barrel-barrel");
  if (etaclass == "ee") pt->AddText("Endcap-endcap");
  if (etaclass == "oo") pt->AddText("Barrel-endcap");
  ...
  gStyle->SetOptTitle(0);
  TPad *p2 = new TPad("p2", "",0.0,0.02,1.0,0.249,0,0,0);
  c1->cd();
  p2->Draw();
  p2->cd();
  p2->SetGridx(kFALSE);
  ...
  ratio_bb->GetXaxis()->SetRangeUser(80,100);
  ratio_bb->GetYaxis()->SetTitleOffset(0.29);
  ratio_bb->GetXaxis()->SetMoreLogLabels();
  ratio_bb->GetXaxis()->SetTitle("m [GeV]");
  ratio_bb->GetYaxis()->SetTitleSize(0.18);
  ratio_bb->GetXaxis()->SetTitleSize(0.20);
  ratio_bb->GetYaxis()->SetLabelSize(0.2);
  ratio_bb->GetYaxis()->SetNdivisions(503);
  ...
  line->DrawLine(80,1,100,1);
  //c1->SaveAs(etaclass+run+"_noBkg.png"); //_DR53X_HCP2012.png");
}
