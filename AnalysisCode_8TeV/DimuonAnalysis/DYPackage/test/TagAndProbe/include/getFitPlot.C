...
RooPlot* getFitPlot(TString INPUTDIR_PREFIX, TString SCENARIO, TString dataMC, TString vartype, TString SCEN_TRIG,TString thisShape,TString whichCan,TString thisBin, TString dataSigBkg) { 
...
//
//
   RooPlot* pad = (RooPlot*)c->GetPrimitive("fit_canvas_"+whichCan);  // "fit_canvas_"+getCanv(whichCan)); //which pad - can be 1, 2, 3
   return pad;
}
