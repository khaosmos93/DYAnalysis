void rochStudy(TString folder, const char* h) {
    ...
    const double fullMCintegral = ttbarNorm+WleptonnuNorm+ZtautauNorm+dibosonNorm+signalNorm;
    ...
    //set up ratio check
    TH1D* hratio = (TH1D*)hsignal->Clone();
    ...
    //TPad *p2 = new TPad("p2", "",0.0,0.0,1.0,0.23,0,0,0);
    c1->cd();
    p2->Draw();
    p2->cd();
    //p2->SetLogx();
    ...
    //hratio->GetXaxis()->SetRangeUser(70,110);
    ...
    hratio->SetMaximum(1.12);
    hratio->SetMinimum(0.88);
