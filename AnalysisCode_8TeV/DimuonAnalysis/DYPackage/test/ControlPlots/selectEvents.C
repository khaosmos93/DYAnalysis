...
//   TH1F* hTrigger = new TH1F("hTrigger","",32,-0.5,31.5);
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {        
    ...
    "runNum/i:evtNum:lumiSec:nTracks0:nCaloTowers0:nPV:nGoodPV:nJets:caloMEx/F:caloMEy:caloSumET:tcMEx:tcMEy:tcSumET:pfMEx:pfMEy:pfSumET:mass:pt:y:phi:pt_1:eta_1:phi_1:scEt_1:scEta_1:scPhi_1:hltMatchBits_1/i:q_1/I:pt_2/F:eta_2:phi_2:scEt_2:scEta_2:scPhi_2:hltMatchBits_2/i:q_2/I:weight/F:nCands/i"
...
// 		   << gen->vmass << endl;
	...
	// loop through dielectrons
        //FIXME alexey
        const int ncands = dielectronArr->GetEntriesFast();
	for(Int_t i=0; i<ncands; i++) {
	  ...
	  if (ee_selection_new_code) {
	    ...
	    if (!doControlPlots && !eeSelector(dielectron,
	    		    escaleCorrType,
...
// 	  if(!passSmurf(dielectron)) continue;  
	  ...
	  if( DYTools::energy8TeV == 1 && !doControlPlots){
	  ...
	  fillData(data, info, dielectron, totalPV, nGoodPV, njets, weightSave); //, ncands);
	  outTree->Fill();
	  if (eemTree) {
	    eemTree->Fill();
