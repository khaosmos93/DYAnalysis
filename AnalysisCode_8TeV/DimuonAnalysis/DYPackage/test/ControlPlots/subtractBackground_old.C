  ...
  if(useTrue2eBgDataDriven || doDDvsMCcomparisonTable)
    {
      ...
      cout << "Name of the source file for data driven bkg " << true2eFName << endl;
      TFile fTrueDataDriven(true2eFName);
      //FIXME SAV temporary
      //assert(unfolding::checkBinningArrays(fTrueDataDriven));
      TMatrixD* true2eBackgroundFromData_pre        = (TMatrixD*)fTrueDataDriven.Get("true2eBackgroundFromData");
      TMatrixD* true2eBackgroundFromDataError_pre     = (TMatrixD*)fTrueDataDriven.Get("true2eBackgroundFromDataError");
      TMatrixD* true2eBackgroundFromDataErrorSyst_pre = (TMatrixD*)fTrueDataDriven.Get("true2eBackgroundFromDataErrorSyst");

/*
      TH2D* tmp = new TH2D(*true2eBackgroundFromData_pre);
      cout << "Initial " << tmp->GetNbinsX() << " " << tmp->GetNbinsY() << endl;
      //FIXME TH2 flips the vixible dimentions!!!
      TMatrixD true2eBackgroundFromData_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());
      TMatrixD true2eBackgroundFromDataError_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());
      TMatrixD true2eBackgroundFromDataErrorSyst_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());

      for (int jx = 0; jx < tmp->GetNbinsY(); jx++) {
        for (int iy = 0; iy < tmp->GetNbinsX(); iy++) {
              //cout << "iy " << iy << " jx " << jx << " " << (*true2eBackgroundFromData_pre)(jx,iy) << " " << tmp->GetBinContent(iy+1,jx+1) << endl;
              true2eBackgroundFromData_pre1(iy,jx) = (*true2eBackgroundFromData_pre)(jx,iy);
              true2eBackgroundFromDataError_pre1(iy,jx) = (*true2eBackgroundFromDataError_pre)(jx,iy);
              true2eBackgroundFromDataErrorSyst_pre1(iy,jx) = (*true2eBackgroundFromDataErrorSyst_pre)(jx,iy);
        }
      }

      TMatrixD true2eBackgroundFromData_tmp(DYTools::nMassBins,nYBinsMax);
      TMatrixD true2eBackgroundFromDataError_tmp(DYTools::nMassBins,nYBinsMax);
      TMatrixD true2eBackgroundFromDataErrorSyst_tmp(DYTools::nMassBins,nYBinsMax);
      //FIXME resize matrices because something is messed up
      //cout << "1:  " << DYTools::nMassBins << " " << DYTools::nYBinsMax << endl;
      //Describe the transformation
      //Number of mass bins in target: 7 (0 - underflow, no overflow); 24 rapidity bins
      //Number of bins in the source: 6 mass bins, 25 rapidity
      // Mass bins - Y axis      
      for (int ix = 0; ix < DYTools::nMassBins; ix++) {
        int jxmy = 0;
        for (int jy = 0; jy < DYTools::nYBinsMax; jy++) {
           if (jy != 24) { 
            if (ix == 0) {
              //add underflow - equal to the first bin
              true2eBackgroundFromData_tmp(0,jxmy) = (*true2eBackgroundFromData_pre)(0,jy);
              true2eBackgroundFromDataError_tmp(0,jxmy) = (*true2eBackgroundFromDataError_pre)(0,jy);
              true2eBackgroundFromDataErrorSyst_tmp(0,jxmy) = (*true2eBackgroundFromDataErrorSyst_pre)(0,jy);
            } else {
              //other bins: p[arallel in mass now, no transformation needed
              true2eBackgroundFromData_tmp(ix,jxmy) = (*true2eBackgroundFromData_pre)(ix-1,jy);
              true2eBackgroundFromDataError_tmp(ix,jxmy) = (*true2eBackgroundFromDataError_pre)(ix-1,jy);
              true2eBackgroundFromDataErrorSyst_tmp(ix,jxmy) = (*true2eBackgroundFromDataErrorSyst_pre)(ix-1,jy);
            }
              jxmy++;
          }
        }
      }
      true2eBackgroundFromData = true2eBackgroundFromData_tmp;
      true2eBackgroundFromDataError = true2eBackgroundFromDataError_tmp;
      true2eBackgroundFromDataErrorSyst = true2eBackgroundFromDataErrorSyst_tmp;
*/
//FIXME do that instead
      true2eBackgroundFromData = (*true2eBackgroundFromData_pre);
      true2eBackgroundFromDataError = (*true2eBackgroundFromDataError_pre);
      true2eBackgroundFromDataErrorSyst = (*true2eBackgroundFromDataErrorSyst_pre);
  ...
  if(useFakeBgDataDriven || doDDvsMCcomparisonTable)
	...
	fakeEEFName.Append(".root");
        ...
        cout << "Filename " << fakeEEFName << endl;
        cout << "Target binning (matrix) X x Y... 7-24: " << DYTools::nMassBins << " " << DYTools::nYBinsMax << endl;
	//assert(unfolding::checkBinningArrays(fFakeDataDriven));
        ...
        TMatrixD* fakeEleBackgroundFromData_pre          = (TMatrixD*)fFakeDataDriven.Get("fakeBackgroundFromData");
        TMatrixD* fakeEleBackgroundFromDataError_pre     = (TMatrixD*)fFakeDataDriven.Get("fakeBackgroundFromDataError");
        TMatrixD* fakeEleBackgroundFromDataErrorSyst_pre = (TMatrixD*)fFakeDataDriven.Get("fakeBackgroundFromDataErrorSyst");
        TH2D* tmp = new TH2D(*fakeEleBackgroundFromData_pre);
        //remembver about TH matrix flip!!!
        cout << "Initial binning 25x7 " << tmp->GetNbinsY() << " " << tmp->GetNbinsX() << endl;
        //FIXME TH2 flips the vixible dimentions!!!
        TMatrixD fakeEleBackgroundFromData_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());
        TMatrixD fakeEleBackgroundFromDataError_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());
        TMatrixD fakeEleBackgroundFromDataErrorSyst_pre1(tmp->GetNbinsX(),tmp->GetNbinsY());

        for (int jx = 0; jx < tmp->GetNbinsY(); jx++) {
          for (int iy = 0; iy < tmp->GetNbinsX(); iy++) {
              //cout << "iy " << iy << " jx " << jx << " " << (*fakeEleBackgroundFromData_pre)(jx,iy) << " " << tmp->GetBinContent(iy+1,jx+1) << endl;
              fakeEleBackgroundFromData_pre1(iy,jx) = (*fakeEleBackgroundFromData_pre)(jx,iy);
              fakeEleBackgroundFromDataError_pre1(iy,jx) = (*fakeEleBackgroundFromDataError_pre)(jx,iy);
              fakeEleBackgroundFromDataErrorSyst_pre1(iy,jx) = (*fakeEleBackgroundFromDataErrorSyst_pre)(jx,iy);
          }
        }

        TMatrixD fakeEleBackgroundFromData_tmp(DYTools::nMassBins,nYBinsMax);
        TMatrixD fakeEleBackgroundFromDataError_tmp(DYTools::nMassBins,nYBinsMax);
        TMatrixD fakeEleBackgroundFromDataErrorSyst_tmp(DYTools::nMassBins,nYBinsMax);
        //FIXME resize matrices because something is messed up
        //cout << "1:  " << DYTools::nMassBins << " " << DYTools::nYBinsMax << endl;
        //Describe the transformation
        //Number of mass bins in target: 7 (0 - underflow, no overflow); 24 rapidity bins
        ...
        for (int ix = 0; ix < DYTools::nMassBins; ix++) {
          ...
          for (int jy = 0; jy < DYTools::nYBinsMax; jy++) {
           if (jy != 24) {
            if (ix == 0) {
              //add underflow - equal to the first bin
              fakeEleBackgroundFromData_tmp(0,jxmy) = (*fakeEleBackgroundFromData_pre)(1,jy);
              fakeEleBackgroundFromDataError_tmp(0,jxmy) = (*fakeEleBackgroundFromDataError_pre)(1,jy);
              fakeEleBackgroundFromDataErrorSyst_tmp(0,jxmy) = (*fakeEleBackgroundFromDataErrorSyst_pre)(1,jy);
            } else {
              //other bins: p[arallel in mass now, no transformation needed
              fakeEleBackgroundFromData_tmp(ix,jxmy) = (*fakeEleBackgroundFromData_pre)(ix,jy);
              fakeEleBackgroundFromDataError_tmp(ix,jxmy) = (*fakeEleBackgroundFromDataError_pre)(ix,jy);
              fakeEleBackgroundFromDataErrorSyst_tmp(ix,jxmy) = (*fakeEleBackgroundFromDataErrorSyst_pre)(ix,jy);
            }
              jxmy++;
            }
          }
        }
        fakeEleBackgroundFromData = fakeEleBackgroundFromData_tmp;
        fakeEleBackgroundFromDataError = fakeEleBackgroundFromDataError_tmp;
        fakeEleBackgroundFromDataErrorSyst = fakeEleBackgroundFromDataErrorSyst_tmp;
/*
        TFile* f = new TFile("ny.root","recreate");
        f->cd();
        TH2D* h = new TH2D(fakeEleBackgroundFromData_tmp);
        TH2D* h2 = new TH2D(fakeEleBackgroundFromData_pre1);
        h->Write("hi");
        h2->Write("hi2");
        f->Close();
*/
  ...
  if (!useTrue2eBgDataDriven) {
    ...
    for (int i=0; i<DYTools::nMassBins; i++)
      for (int j=0; j<DYTools::nYBins[i]; j++) 
        {
           if (i==DYTools::nMassBins-1) cout << i << " " << j << " " << true2eBackground(i,j) << endl;
