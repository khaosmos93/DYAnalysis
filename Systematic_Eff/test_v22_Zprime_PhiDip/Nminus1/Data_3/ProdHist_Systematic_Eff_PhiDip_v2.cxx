...
public:
  ...
  void Fill_Den ( Muon mu1, Bool_t pass1, Muon mu2, Bool_t pass2, Double_t weight )
  {
    ...
    if (pass1) {
      ...
      if ( (fabs(ProbeMu.phi) > 1.3) && (fabs(ProbeMu.phi) < 1.7) ) h_nPixelHits_1p5 ->Fill( ProbeMu.pixelHitsGLB, weight );
    }
...
public:
  ...
  HistogramProducer( Int_t _isMC, TString _FileName_ROOTFileList, Double_t _NormFactor, Int_t _theCut )
  {
    this->isMC = _isMC;
    ...
    this->theCut = _theCut;   //Min

    TString theCuts[6] = { "fabs(mu.dB) < 0.2", "mu.trackerLayersGLB > 5", "mu.pixelHitsGLB > 0", "mu.muonHits > 0", "(mu.TuneP_pTError / mu.TuneP_pT ) < 0.3", "Z' matched station cut" };   //Min

    printf("===============[HistogramProducer]===============\n");
    printf("[isMC = %d] Tag = %s, NormFactor = %lf\n", this->isMC, this->Tag.Data(), this->NormFactor);
    printf("=================================================\n");
    cout << endl;
    cout << "===============[Analyse Dip in Phi]===============" << endl;
    if ( this->theCut == 0 ) cout << "\tEvery cuts are On!" << endl;
    for (Int_t i=0; i<6; ++i) {
      if ( this->theCut == (i+1) ) cout << "\t" << theCuts[i] << " : Off!!!" << endl;
      else cout << "\t" << theCuts[i] << " : On" << endl;
    }
    cout << "==================================================" << endl;
  }
  ...
  void Producer()
  {
    ...
    for(Int_t i=0; i<nTotEvent; i++)
    {
      ...
      if( GenFlag == kTRUE && GenFlag2 == kTRUE)
      {
        ...
        if( ntuple->isTriggered( analyzer->HLT ) && Flag_noBadMuons )
        {
          ...
          Bool_t IsSelected_NUM = this->EventSelection_Zprime_PhiDip_NUM( ntuple, analyzer, MuonCollection, SelectedPair_NUM, SelectedPairPass_NUM );
    ...
    TString theCuts_[7] = { "EveryCuts", "No-dB", "No-trackerLayers", "No-pixelHitsGLB", "No-muonHits", "No-dPtOverPt", "No-ZprimeStationCut" };   //Min

    TFile *f_output = TFile::Open("ROOTFile_Output_Systematic_Eff_"+this->Tag+"_PhiDip_v2_"+theCuts_[this->theCut]+".root", "RECREATE");
...
protected:
  ...
  Bool_t isMuon_PhiDip_NUM( Muon mu, Int_t theCut)   //Min
  {
  ...
  Bool_t EventSelection_Zprime_PhiDip_NUM( NtupleHandle* ntuple, DYAnalyzer* analyzer, vector<Muon> &MuonCollection, MuPair &SelectedPair, pair<Bool_t, Bool_t> &SelectedPairPass )
  {
    Bool_t isPassEventSelection = kFALSE;
    ...
    if( nQMuons >= 2 ) // -- at least 2 muons passing ID -- //
    {
      ...
      if( nPair >= 1 ) // -- at least one pair pass all good pair condition -- //
      {
        ...
        Bool_t p1 = false;
        Bool_t p2 = false;
        if ( this->isMuon_PhiDip_NUM( SelectedPair.First, this->theCut ) ) p1 = true;
        if ( this->isMuon_PhiDip_NUM( SelectedPair.Second, this->theCut ) ) p2 = true;
        SelectedPairPass = make_pair( p1, p2 );
      }
    } // -- end of if( nQMuons >= 2 ) -- //
...
void ProdHist_Systematic_Eff_PhiDip_v2(Int_t _isMC, TString _FileName_ROOTFileList, Double_t _NormFactor, Int_t _theCut)
{
  HistogramProducer *producer = new HistogramProducer( _isMC, _FileName_ROOTFileList, _NormFactor, _theCut );
  producer->Producer();
}
