...
public:
  ...
  void Set_Histograms() {
    ...
    for(iPtBin=0; iPtBin<nPtBin; ++iPtBin) {
      ...
      HistTitle_Pt_E = "pT [" + BinLow+", "+ BinUp +"]    1.2 < |#eta| < 2.2";
...
public:
  ...
  void Producer()
  {
    ...
    for(Int_t i=0; i<nTotEvent; i++)
    {
      ...
      if( GenFlag == kTRUE && GenFlag2 == kTRUE)
      {
        ...
        if( ntuple->isTriggered( analyzer->HLT ) && Flag_noBadMuons )
        {
          ...
          Bool_t IsSelected_DEN = this->EventSelection_Zprime_NUM( ntuple, analyzer, MuonCollection, SelectedPair_BOTH, SelectedPairPass_BOTH ); //EventSelection_Zprime_DEN
          ...
          if( IsSelected_DEN )
          {
            ...
            BinDist_DEN->Fill_Den( mu1, pass1, mu2, pass2, TotWeight);
            Hist_NUM->Fill_Num( mu1, pass1, mu2, pass2, TotWeight);
            BinDist_NUM->Fill_Num( mu1, pass1, mu2, pass2, TotWeight);

            if( !isMC ) // -- data -- //
            {
              if( ntuple->runNum <= 278808 ) {// -- RunBtoF -- //
                Hist_DEN_RunBtoF->Fill_Den( mu1, pass1, mu2, pass2, TotWeight );
                BinDist_DEN_RunBtoF->Fill_Den( mu1, pass1, mu2, pass2, TotWeight);
                Hist_NUM_RunBtoF->Fill_Num( mu1, pass1, mu2, pass2, TotWeight );
                BinDist_NUM_RunBtoF->Fill_Num( mu1, pass1, mu2, pass2, TotWeight);
              }
              else {
                Hist_DEN_RunGtoH->Fill_Den( mu1, pass1, mu2, pass2, TotWeight );
                BinDist_DEN_RunGtoH->Fill_Den( mu1, pass1, mu2, pass2, TotWeight);
                Hist_NUM_RunGtoH->Fill_Num( mu1, pass1, mu2, pass2, TotWeight );
                BinDist_NUM_RunGtoH->Fill_Num( mu1, pass1, mu2, pass2, TotWeight);
              }
            }
          }
    ...
    TFile *f_output = TFile::Open("ROOTFile_Output_Systematic_Eff_"+this->Tag+"_BinHisto_v1.root", "RECREATE");

    Hist_DEN->Save( f_output );
    Hist_NUM->Save( f_output );

    BinDist_DEN->Save( f_output );
    BinDist_NUM->Save( f_output );
    ...
    if( !isMC )
    {
      ...
      BinDist_DEN_RunBtoF->Save( f_output );
      BinDist_NUM_RunBtoF->Save( f_output );

      BinDist_DEN_RunGtoH->Save( f_output );
      BinDist_NUM_RunGtoH->Save( f_output );
    }
...
void ProdHist_Systematic_Eff_BinHisto_v1(Int_t _isMC, Int_t _isZwindow, TString _FileName_ROOTFileList, Double_t _NormFactor)
{
