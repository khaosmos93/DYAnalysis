		...
		for(Int_t i=0; i<nTotEvent; i++)
		{
			...
			if( GenFlag == kTRUE )
			{
				...
				if( ntuple->isTriggered( analyzer->HLT ) )
				{
					...
					Bool_t IsSelected_NUM_DENPlus3 = this->DimuonSelection_Zprime_DENPlusChi2( ntuple, analyzer, QMuonCollection_NUM_DENPlus3, SelectedMuonCollection_NUM_DENPlus3 );
...
protected:
	...
	Bool_t isMuon_NUM_DENPlus3( Muon mu ) // -- single muon cut is same with DENPlus2, but vertex chi2 is applied -- //
	{
		if( mu.isGLB == 1
			&& mu.isTRK == 1
			&& (mu.AbsTrkIso / mu.Inner_pT) < 0.1
