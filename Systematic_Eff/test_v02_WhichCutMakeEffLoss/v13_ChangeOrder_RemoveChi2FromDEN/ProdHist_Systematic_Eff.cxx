...
public:
	...
	void Producer()
	{
				...
				if( ntuple->isTriggered( analyzer->HLT ) )
				{
					...
					for(Int_t j=0; j<(int)MuonCollection.size(); j++)
					{
						if( this->isMuon_DEN( MuonCollection[j] ) && fabs(MuonCollection[j].eta) < 2.4 )
						{
							...
							if( this->isMuon_NUM_DENPlus7( MuonCollection[j] ) )
								QMuonCollection_NUM_DENPlus7.push_back( MuonCollection[j] );
					...
					Bool_t IsSelected_NUM_DENPlus4 = this->DimuonSelection_Zprime_DENPlusChi2( ntuple, analyzer, QMuonCollection_NUM_DENPlus4, SelectedMuonCollection_NUM_DENPlus4 );
					...
					vector< Muon > SelectedMuonCollection_NUM_DENPlus7;
					Bool_t IsSelected_NUM_DENPlus7 = this->DimuonSelection_Zprime_DENPlusChi2( ntuple, analyzer, QMuonCollection_NUM_DENPlus7, SelectedMuonCollection_NUM_DENPlus7 );
					if( IsSelected_NUM_DENPlus7 )
					{
						Muon mu1 = SelectedMuonCollection_NUM_DENPlus7[0];
						Muon mu2 = SelectedMuonCollection_NUM_DENPlus7[1];
						Hist_NUM_DENPlus7->Fill( mu1, mu2, TotWeight);
					}
		...
		Hist_NUM_DENPlus7->Save( f_output );
...
protected:
	...
	Bool_t isMuon_NUM_DENPlus4( Muon mu ) // -- single muon cut is same wiht DENPlus3, but vertex chi2 is applied -- //#
	{
		if( mu.isGLB == 1
			&& mu.isTRK == 1
			&& (mu.AbsTrkIso / mu.Inner_pT) < 0.1
	...
	Bool_t isMuon_NUM_DENPlus5( Muon mu )
	{
		if( mu.isGLB == 1
			&& mu.isTRK == 1
			&& (mu.AbsTrkIso / mu.Inner_pT) < 0.1
	...
	Bool_t isMuon_NUM_DENPlus7( Muon mu )
	{
		if( mu.isGLB == 1
			&& mu.isTRK == 1
			&& (mu.AbsTrkIso / mu.Inner_pT) < 0.1
