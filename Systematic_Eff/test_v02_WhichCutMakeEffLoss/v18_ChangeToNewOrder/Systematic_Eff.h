...
void DrawCanvas_Eff_VariousNUM( TFile* f_input, TString DataType, TString Region, vector< pair<TString, TString> > vec_pair_NUM_Legend )
{
	...
	for(Int_t i_num=0; i_num<nNUM; i_num++)
	{
		...
		if( vec_pair_NUM_Legend[i_num].first == "NUM_DENPlus1" )
		{
			TFile *f_input2 = TFile::Open("/Users/KyeongPil_Lee/Physics/ZprimeAnalysis_80X/Systematic_Eff/test_v02_WhichCutMakeEffLoss/v16_addRatioCut_EntireMassRange/Local/ROOTFile_Output_Systematic_Eff.root");
			f_input2->cd();

			g_temp = (TGraphAsymmErrors*)f_input2->Get(GraphName)->Clone();
		}
	...
	vec_GraphInfo[0]->g->GetYaxis()->SetRangeUser( 0.8, 1.05 );
