...
public:
  ...
  void Producer()
  {
    ...
    for(Int_t i=0; i<nTotEvent; i++)
    {
      ...
      if (!this->isMC) {
        ...
        if ( GenLeptonCollection.size() == 2 ) {
          ...
          if ( this->Tag2 == "DYPowheg_M50to120" ) {   // Z pT < 50 cut
            if ( gen_ZPt < 50 ) {
              GenFlag2 = kTRUE;
            }
          }
    ...
    TFile *f_output = TFile::Open("ROOTFile_Output_Systematic_Eff_"+this->Tag+"_noB2B_v3.root", "RECREATE");
...
void ProdHist_Systematic_Eff_noB2B_v3(Int_t _isMC, TString _FileName_ROOTFileList, Double_t _NormFactor)
{
