	...
	void CheckConsistency( TH1D* h_DEN, TH1D* h_NUM )
	{
		...
		for(Int_t i=0; i<nBin; i++)
		{
			...
			if( DEN < NUM )
			{
				...
				*this->outFile << TString::Format("[%02d bin: %.0lf < M < %.0lf]: DEN < NUM! ... (DEN, NUM) = (%.1lf, %.1lf)", i_bin, LowerEdge, UpperEdge, DEN, NUM) << endl;
				*this->outFile << "\t NUM is set to have same value with DEN" << endl;
