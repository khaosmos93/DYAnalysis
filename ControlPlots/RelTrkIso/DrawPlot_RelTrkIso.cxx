...
void DrawPlot_RelTrkIso(Bool_t isCorrected = kTRUE, TString Type = "MuonPhys", TString HLTname = "IsoMu20_OR_IsoTkMu20")
{
	TTimeStamp ts_start;
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		TH1D* h_RelTrkIso = new TH1D("h_RelTrkIso_"+Tag[i_tup], "", 100, 0, 0.2);
		...
		for(Int_t i=0; i<NEvents; i++)
		{
			...
			if( ntuple->isTriggered( analyzer->HLT ) && GenFlag)
			{
				...
				if( isPassEventSelection == kTRUE )
				{
					...
					h_RelTrkIso->Fill( mu1.trkiso, GenWeight*PUWeight);
					h_RelTrkIso->Fill( mu2.trkiso, GenWeight*PUWeight);
				}
