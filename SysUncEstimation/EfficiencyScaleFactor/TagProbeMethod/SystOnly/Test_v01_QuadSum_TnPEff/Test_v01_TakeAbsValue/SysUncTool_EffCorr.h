...
public:
	...
	void SetUpSysUnc_Total()
	{
		this->isSyst = kTRUE;
		this->SystType = "Total";

		TString AnalyzerPath = gSystem->Getenv("KP_ANALYZER_PATH");
		TString FileName = AnalyzerPath + "/SysUncEstimation/EfficiencyScaleFactor/TagProbeMethod/SystOnly/Test_v01_QuadSum_TnPEff/ROOTFile_Syst_TnPEffs_QuadSumAll.root";

		TH2D *h_Diff_RecoID_Data = Get_Hist_2D( FileName, "h_2D_EffDiff_RecoID_Data" );
		TH2D *h_Diff_RecoID_MC = Get_Hist_2D( FileName, "h_2D_EffDiff_RecoID_MC" );

		TH2D *h_Diff_Iso_Data = Get_Hist_2D( FileName, "h_2D_EffDiff_Iso_Data" );
		TH2D *h_Diff_Iso_MC = Get_Hist_2D( FileName, "h_2D_EffDiff_Iso_MC" );

		TH2D *h_Diff_HLTv4p2_Data = Get_Hist_2D( FileName, "h_2D_EffDiff_HLTv4p2_Data" );
		TH2D *h_Diff_HLTv4p2_MC = Get_Hist_2D( FileName, "h_2D_EffDiff_HLTv4p2_MC" );

		TH2D *h_Diff_HLTv4p3_Data = Get_Hist_2D( FileName, "h_2D_EffDiff_HLTv4p3_Data" );
		TH2D *h_Diff_HLTv4p3_MC = Get_Hist_2D( FileName, "h_2D_EffDiff_HLTv4p3_MC" );
		...
		for(Int_t iter_x = 0; iter_x < nEtaBins; iter_x++)
		{
			for(Int_t iter_y = 0; iter_y < nPtBins; iter_y++)
			{
				...
				EffErr_Sys_RecoID_Data[iter_x][iter_y] = fabs(h_Diff_RecoID_Data->GetBinContent(i_etabin, i_ptbin));
				EffErr_Sys_RecoID_MC[iter_x][iter_y] = fabs(h_Diff_RecoID_MC->GetBinContent(i_etabin, i_ptbin));

				EffErr_Sys_Iso_Data[iter_x][iter_y] = fabs(h_Diff_Iso_Data->GetBinContent(i_etabin, i_ptbin));
				EffErr_Sys_Iso_MC[iter_x][iter_y] = fabs(h_Diff_Iso_MC->GetBinContent(i_etabin, i_ptbin));

				EffErr_Sys_HLTv4p2_Data[iter_x][iter_y] = fabs(h_Diff_HLTv4p2_Data->GetBinContent(i_etabin, i_ptbin));
				EffErr_Sys_HLTv4p2_MC[iter_x][iter_y] = fabs(h_Diff_HLTv4p2_MC->GetBinContent(i_etabin, i_ptbin));

				EffErr_Sys_HLTv4p3_Data[iter_x][iter_y] = fabs(h_Diff_HLTv4p3_Data->GetBinContent(i_etabin, i_ptbin));
				EffErr_Sys_HLTv4p3_MC[iter_x][iter_y] = fabs(h_Diff_HLTv4p3_MC->GetBinContent(i_etabin, i_ptbin));
			}
		}
	...
	void Generate_SmearedEffMap_SystOnly_AllRnd()
	{
		TRandom3 eran;
		eran.SetSeed(0);
		...
		for(Int_t i_map=0; i_map<nEffMap; i_map++)
		{
			for(Int_t iter_x=0; iter_x<nEtaBin; iter_x++)
			{
				for(Int_t iter_y=0; iter_y<nPtBin; iter_y++)
				{
					Eff_RecoID_Data_Smeared[i_map][iter_x][iter_y] = Eff_RecoID_Data[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_RecoID_MC[iter_x][iter_y];
					Eff_RecoID_MC_Smeared[i_map][iter_x][iter_y] = Eff_RecoID_MC[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_RecoID_MC[iter_x][iter_y];

					Eff_Iso_Data_Smeared[i_map][iter_x][iter_y] = Eff_Iso_Data[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_Iso_Data[iter_x][iter_y];
					Eff_Iso_MC_Smeared[i_map][iter_x][iter_y] = Eff_Iso_MC[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_Iso_MC[iter_x][iter_y];

					Eff_HLTv4p2_Data_Smeared[i_map][iter_x][iter_y] = Eff_HLTv4p2_Data[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_HLTv4p2_Data[iter_x][iter_y];
					Eff_HLTv4p2_MC_Smeared[i_map][iter_x][iter_y] = Eff_HLTv4p2_MC[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_HLTv4p2_MC[iter_x][iter_y];

					Eff_HLTv4p3_Data_Smeared[i_map][iter_x][iter_y] = Eff_HLTv4p3_Data[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_HLTv4p3_Data[iter_x][iter_y];
					Eff_HLTv4p3_MC_Smeared[i_map][iter_x][iter_y] = Eff_HLTv4p3_MC[iter_x][iter_y] + eran.Gaus(0.0, 1.0) * EffErr_Sys_HLTv4p3_MC[iter_x][iter_y];
