...
void GetRMS_UnbinnedLikelihoodFit()
{
	...
	for(Int_t i=0; i<nMassBin; i++)
	{
		...
		Double_t RelDiff_min = -0.1;
		Double_t RelDiff_max = 0.1;

		if( Type == "DataMomCorr" || Type == "MCMomCorr" )
		{
			RelDiff_min = -0.5;
			RelDiff_max = 0.5;
		...
		RooRealVar RelDiff("RelDiff","(#sigma_{Central} - #sigma_{Smeared}) / #sigma_{Central}", RelDiff_min, RelDiff_max);
		RooDataSet data("data","data", tree, RelDiff) ; // -- Name of the variable should be same with the branch name in the tree -- //

		// Make plot of binned dataset showing Poisson error bars (RooFit default)
		RooPlot* frame = RelDiff.frame(Title("Relative difference between central and smeared value in "+TString::Format("%2.d", i_bin)+"th mass bin") );
		...
		if( MassBinEdges[i] >= 700 ) // -- after M > 700 GeV -- //
			RMS = data.rmsVar( RelDiff )->getVal();
