...
=============================================
Vegas Parameters
'Relative accuracy (in %)           = ' {_RelUnc:.0f}.0d0
...
def CreateHistogramFile( LowerEdge, UpperEdge ):
	FileName = "histograms_M%.0fto%.0f.txt" % (LowerEdge, UpperEdge)
...
'1.  Z/W pT           ' 50		0d0		250d0		1
'2.  Z/W eta          ' 50		-10d0		10d0		1
'3.  Q_ll inv mass    ' {_BinSize:.0f}		{_LowerEdge:.0f}d0		{_UpperEdge:.0f}d0		1
'4.  l-/lep. pT       ' 50		0d0		100d0		1
'5.  l-/lep. eta      ' 50		-10d0		10d0		1
'6.  l+/neut. pT      ' 50		0d0		100d0		1
'7.  l+/neut. eta     ' 50		-10d0		10d0		1
...
'24. A_FB vs Q_ll     ' 20              66d0            116d0           1
====================================================================================
...
'Display option =' 0
""".format( _LowerEdge=LowerEdge, _UpperEdge=UpperEdge, _BinSize=BinSize )
	)
	f.close()
...
./local_run.sh z {_DirName} {_InputFileName} {_HistogramFileName} {_DirName}.dat ../ 10 >&log_{_DirName}
./finish.sh {_DirName} NLO.{_DirName}.dat
...
def CreateScriptForAllSubmission( BatchFileNames ):
	f = open("qsub_all.sh", "w")
	f.write("#!/bin/bash\n")

	for BatchFileName in BatchFileNames:
		QsubName = BatchFileName.split("_")[1] + "_" + BatchFileName.split("_")[2]
		f.write("qsub -N %s %s\n" % (QsubName, BatchFileName) )

	f.write('\necho "job submission is finished"\n\n')
	f.close()

	print "[qsub_all.sh is created]"
...
for i in range(0, nMassBin):
	...
	Alpha0 = 0;
	InputFileName_DY = CreateInputFile( LowerEdge, UpperEdge, Alpha0, AlphaEff, RelUnc, Order_pQCD, AccFlag, PDFset )
	BatchFileName_DY = CreateBatchJobScript( InputFileName_DY, HistogramFileName )
	BatchFileNames.append( BatchFileName_DY )
