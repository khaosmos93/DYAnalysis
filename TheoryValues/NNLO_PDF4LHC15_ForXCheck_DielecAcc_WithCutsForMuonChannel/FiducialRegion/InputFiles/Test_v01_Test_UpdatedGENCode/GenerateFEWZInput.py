...
class GenerateFEWZInput:
	...
	def Generate_AllBins(self):
		...
		for i in range( 0, len(self.List_BinEdge)-1 ):
			...
			HistFileName = self.Generate_HistFileName( LowerEdge, UpperEdge )
			InputFileName = self.Generate_InputFile( LowerEdge, UpperEdge )
			self.FillList_CMD( InputFileName, HistFileName )

		# -- put spilt jobs in each bash script -- #
		nCMD = len(self.List_cmd_fewz)
		if self.nJob > nCMD:
			print "nJob > nCMD -> nJob is set as same with nCMD"
			self.nJob = nCMD

		nCMDPerJob = int( float(nCMD) / float(self.nJob) )
		print "nJob = %d, nCMD = %d -> nCMDPerJob = %d\n" % (self.nJob, nCMD, nCMDPerJob)

		BatchFileNames = []
		for i in range(0, self.nJob):
			List_CMDPerJob = []
			if i == self.nJob-1:
				List_CMDPerJob = self.List_cmd_fewz[int(i*nCMDPerJob):]
			else:
				List_CMDPerJob = self.List_cmd_fewz[int(i*nCMDPerJob):int((i+1)*nCMDPerJob)]

			BatchFileName = self.Generate_EachBatchJobScript( List_CMDPerJob, i )
	...
	def Print_AllParameters( self ):
		...
		print "\t[# calculation]", len(self.List_BinEdge)-1
		print "\t[# jobs]", self.nJob
...
=============================================""".format( 
	...
	def Generate_EachBatchJobScript( self, List_CMD, index ):
		FileName = self.Make_FileName( self.List_BinEdge[0], self.List_BinEdge[-1] )
		BatchFileName = FileName.replace("input_", "b_").replace("txt", "sh")
		BatchFileName = BatchFileName.replace( ".sh", "_v%02d.sh" % index)
...
"""
	...
	def FillList_CMD(self, InputFileName, HistFileName):
		BatchFileName = InputFileName.replace("input_", "b_").replace("txt", "sh")
		DirName = BatchFileName.split(".sh")[0].split("b_")[1]
		OrderName = self.Get_OrderName()

		cmd_local_run = "./local_run.sh z {_DirName} {_InputFileName} {_HistFileName} {_DirName}.dat ../ {_nCore} >&log_{_DirName}".format(
			_DirName=DirName, _InputFileName=InputFileName, _HistFileName=HistFileName, _nCore = self.nCore)
		cmd_finish = "./finish.sh {_DirName} {_OrderName}.{_DirName}.dat".format( _DirName=DirName, _OrderName=OrderName)

		cmd_fewz = "%s\n%s\n\n" % (cmd_local_run, cmd_finish)
		self.List_cmd_fewz.append( cmd_fewz )
	...
	def Generate_ScriptForAllSubmission( self, BatchFileNames ):
		...
		for BatchFileName in BatchFileNames:
			QsubName = "FEWZ_"+BatchFileName.split(".sh")[0].split("_")[-1]
