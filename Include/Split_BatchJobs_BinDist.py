...
try:
    opts, args = getopt(sys.argv[1:], 'n', ["code=", "sample=", "njob=", "lumi=", "isMC=", "isZwin=","queue=", "outdir="])
    opts = dict(opts)
except:
...
class SplitJobs:
  def __init__(self, _opts):
    ...
    print "+" * 100
    print "Create %d jobs to run %s with lumi = %lf on %s with %s -> queue name = %s" % (self.nJob, self.CodeName, self.Lumi, _TYPE, _TYPE2, self.queue)
  ...
  def CreateWorkSpace_PerJob( self, _iter, _List_ROOTFilesPerJob, List_cmd_qsub, List_cmd_hadd ):
    ...
    cmd_execute = "root -l -b -q '"+ self.CodeName + '++(%d, %d, "%s", %.15lf)' % (self.isMC, self.isZwin, FileName, self.NormFactor) + "'";
    print cmd_execute
