...
try:
    opts, args = getopt(sys.argv[1:], 'n', ["code=", "sample=", "njob=", "lumi=", "isMC=", "queue=", "outdir=", "theCut="])
    opts = dict(opts)
except:
...
class SplitJobs:
  ...
  def CreateWorkSpace_PerJob( self, _iter, _List_ROOTFilesPerJob, List_cmd_qsub, List_cmd_hadd ):
    ...
    cmd_execute = "root -l -b -q '"+ self.CodeName + '++(%d, "%s", %.15lf, %d)' % (self.isMC, FileName, self.NormFactor, self.theCut) + "'";
    print cmd_execute
