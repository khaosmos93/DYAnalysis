...
public:
	...
	TGraphAsymmErrors *g_ratio1; TGraphAsymmErrors *g_ratio2; // -- for the canvas with multiple ratio plot -- //
	...
	void CanvasWithThreeGraphRatioPlot(TGraphAsymmErrors *g1, TGraphAsymmErrors *g2, TGraphAsymmErrors *g3,
									TString Name1, TString Name2, TString Name3, TString Name_Ratio,
									Int_t color1 = kBlue, Int_t color2 = kGreen+1, Int_t color3 = kRed,
									TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME", TString DrawOp3 = "EPSAME")
	{
		c->cd();
		// -- Top Pad -- //
		TopPad = new TPad("TopPad", "TopPad", 0.01, 0.01, 0.99, 0.99);
		...
		// -- Make Ratio plot & Draw it -- //
		this->g_ratio1 = (TGraphAsymmErrors*)g1->Clone();
		MakeRatioGraph(this->g_ratio1, g1, g3);

		this->g_ratio2 = (TGraphAsymmErrors*)g1->Clone();
		MakeRatioGraph(this->g_ratio2, g2, g3);

		g_ratio1->Draw("AEP");
		g_ratio2->Draw("EPSAME");
		...
		// -- X-axis Setting -- //		
		g_ratio1->GetXaxis()->SetTitle( TitleX );
		g_ratio1->GetXaxis()->SetTitleOffset( 0.9 );
		g_ratio1->GetXaxis()->SetTitleSize( 0.2 );
		g_ratio1->GetXaxis()->SetLabelColor(1);
		g_ratio1->GetXaxis()->SetLabelFont(42);
		g_ratio1->GetXaxis()->SetLabelOffset(0.007);
		g_ratio1->GetXaxis()->SetLabelSize(0.15);
		if( isSetNoExpo_MoreLogLabels_X == kTRUE ) { g_ratio1->GetXaxis()->SetMoreLogLabels(); g_ratio1->GetXaxis()->SetNoExponent(); }
		// if( LowerEdge_X != 0 && UpperEdge_X != 0 ) g_ratio1->GetXaxis()->SetRangeUser( LowerEdge_X, UpperEdge_X );
		if( LowerEdge_X != 0 && UpperEdge_X != 0 ) g_ratio1->GetXaxis()->SetLimits( LowerEdge_X, UpperEdge_X );
