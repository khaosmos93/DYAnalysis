...
class SplitJobs:
  ...
  def CreateWorkSpace_PerJob( self, _iter, _List_ROOTFilesPerJob, List_cmd_qsub, List_cmd_hadd ):
    ...
    cmd_sub = 'qsub -q "middleq@compute-0-3.local,middleq@compute-0-4.local,shortq@compute-0-1.local" %s' % (BatchFileName)
    #cmd_sub = "qsub -l hostname=compute-0-10 -V -q %s %s" % (self.queue, BatchFileName)
